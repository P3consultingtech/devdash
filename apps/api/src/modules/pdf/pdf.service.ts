  ...
  return new Promise((resolve, reject) => {
    ...
    if (businessProfile) {
      doc
        .fontSize(11)
        .font('Helvetica-Bold')
        .text(businessProfile.businessName, rightCol, y, { width: 200, align: 'right' });
      y += 16;
      doc.fontSize(9).font('Helvetica');
      if (businessProfile.street) {
        doc.text(businessProfile.street, rightCol, y, { width: 200, align: 'right' });
        y += 12;
      }
      if (businessProfile.city) {
        const addr = [businessProfile.postalCode, businessProfile.city, businessProfile.province]
          .filter(Boolean)
          .join(' ');
        doc.text(addr, rightCol, y, { width: 200, align: 'right' });
        y += 12;
      }
      if (businessProfile.partitaIva) {
        doc.text(`P.IVA: ${businessProfile.partitaIva}`, rightCol, y, {
          width: 200,
          align: 'right',
        });
        y += 12;
      }
      if (businessProfile.codiceFiscale) {
        doc.text(`C.F.: ${businessProfile.codiceFiscale}`, rightCol, y, {
          width: 200,
          align: 'right',
        });
        y += 12;
      }
      if (businessProfile.pec) {
        doc.text(`PEC: ${businessProfile.pec}`, rightCol, y, { width: 200, align: 'right' });
        y += 12;
      }
    }
    ...
    if (invoice.client.street) {
      doc.fontSize(9).text(invoice.client.street, 50, y);
      y += 12;
    }
    if (invoice.client.city) {
      const addr = [invoice.client.postalCode, invoice.client.city, invoice.client.province]
        .filter(Boolean)
        .join(' ');
      doc.text(addr, 50, y);
      y += 12;
    }
    if (invoice.client.partitaIva) {
      doc.text(`P.IVA: ${invoice.client.partitaIva}`, 50, y);
      y += 12;
    }
    if (invoice.client.codiceFiscale) {
      doc.text(`C.F.: ${invoice.client.codiceFiscale}`, 50, y);
      y += 12;
    }
    if (invoice.client.codiceDestinatario) {
      doc.text(`Cod. Dest.: ${invoice.client.codiceDestinatario}`, 50, y);
      y += 12;
    }
    if (invoice.client.pec) {
      doc.text(`PEC: ${invoice.client.pec}`, 50, y);
      y += 12;
    }
    ...
    doc.text(formatCurrency(invoice.subtotal, 'it'), totalsValX, y, {
      width: totalsW,
      align: 'right',
    });
    y += 14;
    ...
    if (invoice.applyCassa && invoice.cassaAmount > 0) {
      ...
      doc.text(formatCurrency(invoice.cassaAmount, 'it'), totalsValX, y, {
        width: totalsW,
        align: 'right',
      });
      y += 14;
    ...
    if (invoice.ivaRate > 0) {
      ...
      doc.text(formatCurrency(invoice.ivaAmount, 'it'), totalsValX, y, {
        width: totalsW,
        align: 'right',
      });
      y += 14;
    ...
    if (invoice.applyBollo && invoice.bolloAmount > 0) {
      ...
      doc.text(formatCurrency(invoice.bolloAmount, 'it'), totalsValX, y, {
        width: totalsW,
        align: 'right',
      });
      y += 14;
    ...
    doc.text(formatCurrency(invoice.grossTotal, 'it'), totalsValX, y, {
      width: totalsW,
      align: 'right',
    });
    y += 14;
    ...
    if (invoice.applyRitenuta && invoice.ritenutaAmount > 0) {
      doc.font('Helvetica');
      doc.text(`Ritenuta d'acconto (${invoice.ritenutaRate}%):`, totalsX - 30, y, {
        width: 130,
        align: 'right',
      });
      doc.text(`-${formatCurrency(invoice.ritenutaAmount, 'it')}`, totalsValX, y, {
        width: totalsW,
        align: 'right',
      });
      y += 14;
      ...
      doc.text(formatCurrency(invoice.netPayable, 'it'), totalsValX, y, {
        width: totalsW,
        align: 'right',
      });
      y += 20;
    ...
    if (invoice.paymentTerms || businessProfile?.iban) {
      ...
      if (businessProfile?.iban) {
        doc.text(`IBAN: ${businessProfile.iban}`, 50, y);
        y += 12;
      }
    }
